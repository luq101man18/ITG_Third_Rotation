...
const FilterView = ({ products, setProducts, displayFilter }) => {
    ...
    const savePriceRangeValues = async (value: any) => {
        try {
            ...
            await AsyncStorage.setItem('price-range-values-searchedProducts', jsonValue);
        } catch (e) {
    ...
    const getPriceRangeValues = async () => {
        try {
            const jsonValue = await AsyncStorage.getItem('price-range-values-searchedProducts');
